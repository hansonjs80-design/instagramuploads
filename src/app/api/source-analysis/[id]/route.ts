...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const analysis = await getSourceAnalysis(id);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ analysis: await updateSourceClassification(id, body.classification) });
  } catch (error) {
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const current = await getSourceAnalysis(id);
    ...
    return NextResponse.json({ analysis: await updateSourceClassification(id, classification) });
  } catch (error) {
