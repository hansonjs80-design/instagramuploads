...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { contentId?: unknown; outputType?: unknown; caption?: unknown; images?: unknown; confirmed?: unknown; republish?: unknown };
    ...
    const outputType = body.outputType === "INSTAGRAM_EN" ? "INSTAGRAM_EN" : "INSTAGRAM_KR";
    const job = await createPublishJob({ contentId: body.contentId, outputType, caption: body.caption.slice(0, 2200), images: body.images, confirmed: body.confirmed === true, republish: body.republish === true });
