...
export async function GET() { return NextResponse.json({ account: await getInstagramAccount(), configuredMode: getPublishMode() }); }
...
export async function POST() {
  ...
  return NextResponse.json({ account: await saveInstagramAccount({ ...remote, accessToken: "mock-access-token", tokenType: "mock", expiresAt: null, scopes, publishMode: "MOCK" }) });
}

export async function DELETE() { await disconnectInstagramAccount(); return NextResponse.json({ disconnected: true }); }
