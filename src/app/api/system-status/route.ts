...
export async function GET() {
  ...
  const database = await getDatabaseProviderInfo();
  ...
  const instagram = await getInstagramAccount();
  ...
  try { const app = new URL(getAppBaseUrl()); appUrlStatus = app.protocol === "https:" || process.env.NODE_ENV !== "production" ? "READY" : "ERROR"; appUrlMessage = app.toString(); } catch {}
  items.push({ name: "Application URL", status: appUrlStatus, message: appUrlMessage });
  const ownerReady = Boolean(process.env.STUDIO_OWNER_EMAIL && process.env.STUDIO_PASSWORD_HASH);
  const adminReady = Boolean(process.env.STUDIO_ADMIN_USERNAME && process.env.STUDIO_ADMIN_PASSWORD_HASH);
  const authReady = (ownerReady || adminReady) && (process.env.STUDIO_SESSION_SECRET?.length ?? 0) >= 32;
  items.push({ name: "Authentication", status: authReady ? "READY" : "ERROR", message: authReady ? "Server-side administrator authentication configured" : "관리자 또는 소유자 계정과 32자 이상 session secret가 필요합니다." });
