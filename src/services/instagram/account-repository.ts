...
export function getInstagramAccount(): InstagramAccount | null {
  const row = getDatabase().prepare("SELECT * FROM instagram_accounts WHERE connection_status != 'DISCONNECTED' ORDER BY connected_at DESC LIMIT 1").get() as AccountRow | undefined;
  return row ? map(row) : null;
}

export function getAccountToken(id: string): string {
  const row = getDatabase().prepare("SELECT encrypted_access_token, publish_mode FROM instagram_accounts WHERE id = ?").get(id) as { encrypted_access_token: string; publish_mode: PublishMode } | undefined;
...
export function saveInstagramAccount(input: { instagramUserId: string; username: string; accountType: InstagramAccount["accountType"]; profilePictureUrl: string; accessToken: string; tokenType: string; expiresAt: string | null; scopes: string[]; publishMode: PublishMode }): InstagramAccount {
  const now = new Date().toISOString();
  const id = randomUUID();
  const encrypted = input.publishMode === "MOCK" ? "mock-token-not-a-credential" : encryptToken(input.accessToken);
  getDatabase().prepare(`INSERT INTO instagram_accounts
    ...
    .run(id, input.instagramUserId, input.username, input.accountType, input.profilePictureUrl, encrypted, input.tokenType, input.expiresAt, JSON.stringify(input.scopes), input.publishMode, now, now);
  return getInstagramAccount()!;
}

export function disconnectInstagramAccount(): void {
  getDatabase().prepare("UPDATE instagram_accounts SET connection_status = 'DISCONNECTED', encrypted_access_token = '', last_validated_at = ?").run(new Date().toISOString());
}

export function markAccountValidated(id: string): void {
  getDatabase().prepare("UPDATE instagram_accounts SET last_validated_at = ?, connection_status = 'CONNECTED' WHERE id = ?").run(new Date().toISOString(), id);
}
