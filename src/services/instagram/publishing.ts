...
export async function getPublishJob(id: string, database?: AppDatabase): Promise<PublishJob | null> {
  const db = database ?? await getDatabase();
  const row = await db.get<JobRow>(`SELECT j.*, a.username, (SELECT COUNT(*) FROM instagram_publish_assets x WHERE x.publish_job_id=j.id) card_count, p.permalink FROM instagram_publish_jobs j JOIN instagram_accounts a ON a.id=j.account_id LEFT JOIN instagram_published_posts p ON p.publish_job_id=j.id WHERE j.id=?`, [id]);
  return row ? mapJob(row) : null;
}

export async function getLatestPublishJobByContent(contentId: string, outputType: "INSTAGRAM_KR" | "INSTAGRAM_EN" = "INSTAGRAM_KR"): Promise<PublishJob | null> {
  const db = await getDatabase();
  const row = await db.get<JobRow>(`SELECT j.*, a.username, (SELECT COUNT(*) FROM instagram_publish_assets x WHERE x.publish_job_id=j.id) card_count, p.permalink FROM instagram_publish_jobs j JOIN instagram_accounts a ON a.id=j.account_id LEFT JOIN instagram_published_posts p ON p.publish_job_id=j.id WHERE j.content_id=? AND j.output_type=? ORDER BY j.created_at DESC LIMIT 1`, [contentId, outputType]);
  return row ? mapJob(row) : null;
}
...
export async function createPublishJob(input: { contentId: string; outputType: "INSTAGRAM_KR" | "INSTAGRAM_EN"; caption: string; images: string[]; confirmed: boolean; republish?: boolean }): Promise<PublishJob> {
  ...
  const instagramOutput = content ? await getInstagramOutput(input.contentId, input.outputType) : null;
  ...
  const db = await getDatabase();
  const existing = await db.get<{ id: string; status: PublishJobStatus }>("SELECT id, status FROM instagram_publish_jobs WHERE idempotency_key = ?", [idempotencyKey]);
  ...
  await db.transaction(async (transaction) => {
    await transaction.run(`INSERT INTO instagram_publish_jobs (id, account_id, content_id, content_version_id, output_type, idempotency_key, status, caption, quality_score, brand_score, created_at, confirmed_at) VALUES (?, ?, ?, ?, ?, ?, 'PREPARING', ?, ?, ?, ?, ?)`,
      [jobId, account.id, input.contentId, version, input.outputType, input.republish ? `${idempotencyKey}-${randomUUID()}` : idempotencyKey, input.caption, instagramOutput.instagramEngine.quality.total, instagramOutput.instagramEngine.quality.scores.brand, now, now]);
    for (let index = 0; index < decoded.length; index++) {
      const path = join(directory, `${String(index + 1).padStart(2, "0")}.jpg`);
      await writeFile(path, decoded[index].bytes);
      await transaction.run(`INSERT INTO instagram_publish_assets (id, publish_job_id, card_id, order_index, local_path, jpeg_path, public_url, public_storage_key, content_hash, created_at) VALUES (?, ?, ?, ?, ?, ?, '', ?, ?, ?)`,
        [randomUUID(), jobId, String(index + 1), index, path, path, `${jobId}-${String(index + 1).padStart(2, "0")}.jpg`, decoded[index].hash, now]);
    }
  });
  return (await getPublishJob(jobId, db))!;
}

async function setStatus(database: AppDatabase, id: string, status: PublishJobStatus, extras: { carousel?: string; media?: string; nextPoll?: string | null } = {}) {
  await database.run(`UPDATE instagram_publish_jobs SET status=?, started_at=COALESCE(started_at, ?), carousel_container_id=COALESCE(?, carousel_container_id), instagram_media_id=COALESCE(?, instagram_media_id), next_poll_at=? WHERE id=?`,
    [status, new Date().toISOString(), extras.carousel ?? null, extras.media ?? null, extras.nextPoll ?? null, id]);
}

export async function advancePublishJob(id: string): Promise<PublishJob> {
  const db = await getDatabase();
  const raw = await db.get<{ account_id: string; content_id: string; content_version_id: string; output_type: "INSTAGRAM_KR" | "INSTAGRAM_EN"; status: PublishJobStatus; caption: string; carousel_container_id: string | null; next_poll_at: string | null }>("SELECT * FROM instagram_publish_jobs WHERE id=?", [id]);
  if (!raw) throw new Error("게시 작업을 찾을 수 없습니다.");
  if (["PUBLISHED", "FAILED"].includes(raw.status)) return (await getPublishJob(id, db))!;
  const account = await getInstagramAccount();
  ...
  const assets = await db.all<{ id: string; jpeg_path: string; public_storage_key: string; public_url: string; child_container_id: string | null; container_status: string }>("SELECT * FROM instagram_publish_assets WHERE publish_job_id=? ORDER BY order_index", [id]);
  try {
    if (raw.status === "PREPARING") await setStatus(db, id, "VALIDATING");
      ...
      await setStatus(db, id, "UPLOADING_ASSETS");
      ...
      for (const asset of assets) {
        ...
        await db.run("UPDATE instagram_publish_assets SET public_url=? WHERE id=?", [url, asset.id]);
      }
      await setStatus(db, id, "CREATING_CHILD_CONTAINERS");
      ...
      for (const asset of assets) {
        ...
        await db.run("UPDATE instagram_publish_assets SET child_container_id=?, container_status='IN_PROGRESS' WHERE id=?", [child, asset.id]);
      }
      await setStatus(db, id, "WAITING_CHILDREN", { nextPoll: new Date(Date.now() + (account.publishMode === "MOCK" ? 0 : 60_000)).toISOString() });
    } else if (raw.status === "WAITING_CHILDREN") {
      if (raw.next_poll_at && new Date(raw.next_poll_at).getTime() > Date.now()) return (await getPublishJob(id, db))!;
      let complete = true;
      for (const asset of assets) {
        const status = await provider.getContainerStatus(asset.child_container_id!, token);
        await db.run("UPDATE instagram_publish_assets SET container_status=? WHERE id=?", [status, asset.id]);
      ...
      await setStatus(db, id, complete ? "CREATING_CAROUSEL" : "WAITING_CHILDREN", { nextPoll: complete ? null : new Date(Date.now() + 60_000).toISOString() });
      ...
      await setStatus(db, id, "WAITING_CAROUSEL", { carousel, nextPoll: new Date(Date.now() + (account.publishMode === "MOCK" ? 0 : 60_000)).toISOString() });
    } else if (raw.status === "WAITING_CAROUSEL") {
      if (raw.next_poll_at && new Date(raw.next_poll_at).getTime() > Date.now()) return (await getPublishJob(id, db))!;
      const status = await provider.getContainerStatus(raw.carousel_container_id!, token);
      if (["ERROR", "EXPIRED"].includes(status)) throw new InstagramProviderError("CONTAINER_ERROR");
      await setStatus(db, id, status === "FINISHED" || status === "PUBLISHED" ? "READY_TO_PUBLISH" : "WAITING_CAROUSEL", { nextPoll: new Date(Date.now() + 60_000).toISOString() });
    } else if (raw.status === "READY_TO_PUBLISH") await setStatus(db, id, "PUBLISHING");
      ...
      const now = new Date().toISOString();
      await db.transaction(async (transaction) => {
        await transaction.run("UPDATE instagram_publish_jobs SET status='PUBLISHED', instagram_media_id=?, completed_at=?, next_poll_at=NULL WHERE id=?", [result.mediaId, now, id]);
        const job = (await getPublishJob(id, transaction))!;
        await transaction.run(`INSERT INTO instagram_published_posts (id, instagram_account_id, content_id, content_version_id, output_type, publish_job_id, instagram_media_id, permalink, caption, card_count, published_at, quality_score, brand_score, source_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PUBLISHED')`,
          [randomUUID(), account.id, raw.content_id, raw.content_version_id, raw.output_type, id, result.mediaId, result.permalink, raw.caption, assets.length, now, job.qualityScore, job.brandScore, raw.content_id]);
      });
    }
  } catch (error) {
    ...
    await db.run("UPDATE instagram_publish_jobs SET status='FAILED', failed_at=?, error_code=?, error_message=? WHERE id=?", [new Date().toISOString(), code, message.slice(0, 500), id]);
  }
  return (await getPublishJob(id, db))!;
}

export async function listPublishedPosts() {
  const db = await getDatabase();
  return db.all(`SELECT p.id, p.instagram_media_id "mediaId", p.output_type "outputType", p.permalink, p.caption, p.card_count "cardCount", p.published_at "publishedAt", p.quality_score "qualityScore", p.brand_score "brandScore", p.status, a.username, c.original_title title, c.expert_name "expertName" FROM instagram_published_posts p JOIN instagram_accounts a ON a.id=p.instagram_account_id JOIN content_items c ON c.id=p.content_id ORDER BY p.published_at DESC`);
}
